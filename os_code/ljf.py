...
class LJF:
    ...
    def ljf(self):
        # 1. Sort the processes by burst time
        self.processes.sort(key=lambda x: x.burst, reverse=True)
        # 2. Sort the processes by arrival time
        ...
        for x in range(1, self.n):
            ...
            # 6. Sort the processes by burst time (current index to last finding index)
            self.processes[x:sort_end_index] = sorted(self.processes[x:sort_end_index], key=lambda x: x.burst, reverse=True)
...
if __name__ == '__main__':
    ...
    for x in range(0, ljf.n):
        print(f"{ljf.processes[x].pid:<10}{ljf.processes[x].arrival:<10}{ljf.processes[x].burst:<10}{ljf.processes[x].waiting:<10}{ljf.processes[x].turnaround:<10}{ljf.processes[x].service:<10}")
    print(f"""
